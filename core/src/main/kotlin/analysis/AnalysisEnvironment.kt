...
class AnalysisEnvironment(val messageCollector: MessageCollector, val analysisPlatform: Platform) : Disposable {
    ...
    fun createResolutionFacade(environment: KotlinCoreEnvironment): Pair<DokkaResolutionFacade, DokkaResolutionFacade> {
        ...
        val targetPlatform = when (analysisPlatform) {
            ...
            Platform.jvm -> JvmPlatforms.defaultJvmPlatform // TODO: fix target version
        }

        val library = object : LibraryModuleInfo {
            override val analyzerServices: PlatformDependentAnalyzerServices =
                JvmPlatformAnalyzerServices // todo: make this relative to platform
            override val name: Name = Name.special("<library>")
            override val platform: TargetPlatform = targetPlatform
            override fun dependencies(): List<ModuleInfo> = listOf(this)
            override fun getLibraryRoots(): Collection<String> = classpath.map { it.absolutePath }
        }
        ...
        val resolverForProject = when (analysisPlatform) {
            Platform.jvm -> {
                builtIns = JvmBuiltIns(projectContext.storageManager, JvmBuiltIns.Kind.FROM_CLASS_LOADER) // TODO we should use FROM_DEPENDENCIES
    ...
    private fun createCommonResolverForProject(
        ...
        return object : AbstractResolverForProject<ModuleInfo>(
            ...
            override fun builtInsForModule(module: ModuleInfo): KotlinBuiltIns = DefaultBuiltIns()
    ...
    private fun createJsResolverForProject(
        ...
        return object : AbstractResolverForProject<ModuleInfo>(
            ...
            override fun createResolverForModule(
                ...
                modulesContent(moduleInfo),
                this,
                LanguageVersionSettingsImpl.DEFAULT // TODO fix this
            )

            override fun builtInsForModule(module: ModuleInfo): KotlinBuiltIns = DefaultBuiltIns() //TODO?
...
//                descriptor: ModuleDescriptor,
//                moduleInfo: ModuleInfo
//            ): ResolverForModule = DokkaNativeAnalyzerFacade.createResolverForModule(
//                descriptor as ModuleDescriptorImpl,
//                projectContext.withModule(descriptor),
//                modulesContent(moduleInfo),
//                this,
...
//            )
//
//            override fun builtInsForModule(module: ModuleInfo): KotlinBuiltIns = DefaultBuiltIns()//KonanBuiltIns(projectContext.storageManager) //TODO?
//
//            override fun sdkDependency(module: ModuleInfo): ModuleInfo? = null
//        }
//    }
    ...
    private fun createNativeResolverForProject(
        ...
        return object : AbstractResolverForProject<ModuleInfo>(
            ...
            override fun createResolverForModule(
                descriptor: ModuleDescriptor,
                moduleInfo: ModuleInfo
            ): ResolverForModule = NativeResolverForModuleFactory(
